...
show_access_info() {
    ...
    echo "======================="
    echo -e "🏥 Health Check: ${GREEN}kubectl port-forward -n ${NAMESPACE:-$RELEASE_NAME} svc/$RELEASE_NAME-backend 3001:3001 && curl https://backend.fortunewhisper.com/api/monitoring/health${NC}"
    echo -e "📈 Metrics: ${GREEN}kubectl port-forward -n ${NAMESPACE:-$RELEASE_NAME} svc/$RELEASE_NAME-backend 3001:3001 && curl https://backend.fortunewhisper.com/api/monitoring/metrics${NC}"
}
