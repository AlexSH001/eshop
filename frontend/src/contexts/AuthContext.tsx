...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const checkAuth = async () => {
      ...
      if (token && storedUser) {
        try {
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://fortunewhisper.com/backend/api'}/auth/me`, {
            headers: {
          ...
          if (response.ok) {
            ...
            if (refreshTokenValue) {
              try {
                const refreshResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://fortunewhisper.com/backend/api'}/auth/refresh`, {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                  },
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://fortunewhisper.com/backend/api'}/auth/login`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (sessionId) {
        try {
          const mergeResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://fortunewhisper.com/backend/api'}/cart/merge`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  const signup = async (email: string, password: string, firstName: string, lastName: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://fortunewhisper.com/backend/api'}/auth/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const refreshToken = async (): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://fortunewhisper.com/backend/api'}/auth/refresh`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
