...
export function AdminProvider({ children }: { children: ReactNode }) {
  ...
  const refreshToken = async (): Promise<boolean> => {
    ...
    try {
      const response = await fetch('https://backend.fortunewhisper.com/api/auth/refresh', {
        method: 'POST',
        headers: {
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      const res = await fetch('https://backend.fortunewhisper.com/api/auth/admin/login', {
        method: 'POST',
