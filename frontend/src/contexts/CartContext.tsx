...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const loadCart = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://10.170.0.4:3001/api'}/cart`, {
          headers: {
  ...
  const addItem = async (product: Omit<CartItem, 'quantity' | 'id' | 'productId'> & { id: number }) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://10.170.0.4:3001/api'}/cart/items`, {
        method: 'POST',
        headers: {
  ...
  const removeItem = async (id: number) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://10.170.0.4:3001/api'}/cart/items/${id}`, {
        method: 'DELETE',
        headers: {
          ...(token && { Authorization: `Bearer ${token}` }),
          ...(sessionId && { 'X-Session-ID': sessionId }),
