...
export default function AdminProductsPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await fetch('https://10.170.0.4/api/categories');
  ...
  const handleImageUpload = async (files: FileList) => {
    ...
    try {
      ...
      const res = await makeAuthenticatedRequest('https://10.170.0.4/api/upload/product-images', {
        method: 'POST',
        credentials: 'include',
  ...
  const removeImage = async (imageUrl: string) => {
    try {
      ...
      const res = await makeAuthenticatedRequest('https://10.170.0.4/api/upload/product-image', {
        method: 'DELETE',
        headers: {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const res = await fetch('https://10.170.0.4/api/products');
  ...
  const handleEditProduct = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`https://10.170.0.4/api/products/${editingProduct.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleDeleteProduct = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`https://10.170.0.4/api/products/${deletingProduct.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
                ...
                <tr key={product.id} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    <Button size="sm" variant="outline" onClick={async () => {
                      ...
                      try {
                        const res = await fetch(`https://10.170.0.4/api/products/${product.id}`, {
                          credentials: 'include'
                        });
                        if (res.ok) {
