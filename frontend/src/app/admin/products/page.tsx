...
export default function AdminProductsPage() {
  ...
  const handleImageUpload = async (files: FileList) => {
    ...
    try {
      ...
      const res = await makeAuthenticatedRequest('http://10.170.0.4:3001/api/upload/product-images', {
        method: 'POST',
        credentials: 'include',
  ...
  const removeImage = async (imageUrl: string) => {
    try {
      ...
      const res = await makeAuthenticatedRequest('http://10.170.0.4:3001/api/upload/product-image', {
        method: 'DELETE',
        headers: {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const res = await fetch('http://10.170.0.4:3001/api/products');
  ...
  const handleEditProduct = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`http://10.170.0.4:3001/api/products/${editingProduct.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleDeleteProduct = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`http://10.170.0.4:3001/api/products/${deletingProduct.id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
