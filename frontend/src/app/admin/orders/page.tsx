...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const res = await fetch('http://10.170.0.4:3001/api/orders/authenticated', {
        headers: {
  ...
  const handleEditOrder = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://10.170.0.4:3001/api/orders/${editingOrder.id}/status`, {
        method: 'PUT',
        headers: { 
          'Authorization': `Bearer ${adminToken}`,
          'Content-Type': 'application/json' 
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://10.170.0.4:3001/api/orders/${deletingOrder.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${adminToken}`
        },
