...
export const fetchCategories = async () => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://backend.fortunewhisper.com/api'}/categories`);
...
export const fetchProducts = async () => {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://backend.fortunewhisper.com/api'}/products?limit=50`, {
...
export const normalizeImageUrl = (url: string): string => {
    ...
    if (url.startsWith('/uploads/')) {
        return `https://backend.fortunewhisper.com${url}`;
    }
